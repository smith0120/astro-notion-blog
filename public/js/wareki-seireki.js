...
const eraEndDates = {
	"令和": null,
	"平成": new Date("2019-04-30"),
	"昭和": new Date("1989-01-07"),
	"大正": new Date("1926-12-24"),
	"明治": new Date("1912-07-29")
};
...
function populateYearsByEra(era) {
	...
	const startYear = eraStartDates[era].getFullYear();
	const endYear = eraEndDates[era] ? eraEndDates[era].getFullYear() : new Date().getFullYear();
	const maxYear = endYear - eras[era];
...
function updateWarekiFromSeireki() {
	...
	for (const [name, offset] of Object.entries(eras)) {
		if (isSameOrAfter(date, eraStartDates[name])) {
...
function updateSeirekiFromWareki() {
	...
	if (!isSameOrAfter(date, eraStartDates[era])) {
